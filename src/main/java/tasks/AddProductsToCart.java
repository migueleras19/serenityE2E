...
public class AddProductsToCart implements Task {
    ...
    public <T extends Actor> void performAs(T actor) {
        actor.attemptsTo(WaitUntil.the(ProductPage.ADD_TO_CART_1, isVisible()).forNoMoreThan(4).seconds());
        actor.attemptsTo(Click.on(ProductPage.ADD_TO_CART_1));
        actor.attemptsTo(WaitUntil.the(ProductPage.ADD_TO_CART_2, isVisible()).forNoMoreThan(4).seconds());
        actor.attemptsTo(Click.on(ProductPage.ADD_TO_CART_2));
        actor.attemptsTo(
                WaitUntil.the(ProductPage.CART_BUTTON, isVisible()).forNoMoreThan(4).seconds(),
                Click.on(ProductPage.CART_BUTTON),
                WaitUntil.the(ProductPage.CHECKOUT_BUTTON, isVisible()).forNoMoreThan(4).seconds(),
